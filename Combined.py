...
analysis_dictt=0
drow_score_list=0
secondcount=int(time.time())

def telegramapicall(message):
    global secondcount
    if((int(time.time())-secondcount)>5):
        requests.get("https://api.telegram.org/bot5338165836:AAH16IjYfKsGVo5fLj6abr_SupLrEJzJcGY/sendMessage?chat_id=-615893429&text="+message)
        secondcount=int(time.time())
...
def mainappfun():
    ...
    #############NEWC0DE
        ...
        if (rula != "NULL") and (reba != "NULL"):
            if int(rula)>3:
                ...
                telegramapicall("Warning Posture not proper in upper body")
            else:
            ...
            if int(reba)>4:
                ...
                telegramapicall("Warning Posture not proper in entire body")
            else:
                print("Rapid Entire Body Score : "+reba)
        else:
            print("Posture Incorrect")
            telegramapicall("Warning Posture Incorrect")
    ...
    def drowsiness(frame):
        ...
        if(score>15):
            ...
            print("FEELIN SLEEPY")
            telegramapicall("Feeling Sleepy? Try to take a break!")
            if(thicc<16):
                thicc= thicc+2
            else:
                thicc=thicc-2
    ...
    while (1):
        ...
        try:
            ...
            if(int(neck_inclination)>35):
                telegramapicall("Warning: NECK leaned too much RIGHT, please correct your NECK position")
            if(int(neck_inclination)<24):
                telegramapicall("Warning: NECK leaned too much LEFT, please correct your NECK position")
            if(int(torso_inclination)>13):
                telegramapicall("Warning: TORSO leaned too much RIGHT, please correct your TORSO position")
            if(int(torso_inclination)<15):
                telegramapicall("Warning: TORSO leaned too much LEFT, please correct your TORSO position")
...
page_names_to_funcs = {
    "Face Train": facetrainfun,
    "Detection Part": mainappfun
}
